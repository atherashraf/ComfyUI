    ...
    def __get_mask(
        ...
        if len(masks) == 0:
            ...
            for mask in masks:
                ...
                if mask_view.ndim == 3 and mask_view.shape[-1] == 1:
                    mask_view = np.squeeze(mask_view, axis=-1)
                elif mask_view.ndim != 2:
                    raise ValueError(f"Unexpected mask shape: {mask_view.shape}")

                # Build condition with shape (H, W, 4)
                condition = np.repeat(
                    (mask_view > confidence)[..., None],
                    4,
                    axis=-1
                )
        ...
        if refine_mask:
            ...
            if bbox is not None:
                ...
                updated_mask_image = Image.new("RGBA", image.size, (0, 0, 0, 0))
