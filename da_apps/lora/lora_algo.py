...
from da_apps.lora.steps.step2_face_centered_cropping import yolo_face_hair_crop
...
if __name__ == "__main__":
    concept = "Aija"        # token will be [concept]
    repeats ="1"
    token = concept
    current_step = 1
    ...
    data_dir = Path(rf"D:\Documents\general\downloads\{concept}")
    # Step 2: crop
    if current_step <= 2:
        # yolo_fullbody_crop(data_dir, lora_img_dir, target_size=1024)
        yolo_face_hair_crop(data_dir, lora_img_dir, target_size=1024,)
    if current_step <= 3:
